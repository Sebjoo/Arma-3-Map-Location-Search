...
switch _mode do
{
    case "init":
    {
        ...
        _grpSearch ctrlsetposition [X_GRID - W_GRP,Y_GRID + 1.5 * H_GRID,W_GRP,H_GRP];
        _grpSearch ctrlcommit 0;
        _grpSearch ctrladdeventhandler ["MouseEnter",{systemchat str _this;}];
        ...
        _cbShow ctrlsetposition [0.1 * W_GRID,0,1 * W_GRID,1 * H_GRID];
        _cbShow ctrlcommit 0;
        _cbShow ctrlsettooltip "Toggle City Search";
        _cbShow cbsetchecked true;
        _cbShow ctrladdeventhandler ["CheckedChanged",{["toggleVisibility", _this] spawn SELF;}];
        _btnEndSearch = _map ctrlCreate ["RscActivePicture", IDC_RSCMAP_MRKENDSEARCH, _grpSearch];
        _btnEndSearch ctrlSetPosition [W_GRP - 1 * W_GRID,0.1 * H_GRID,1 * W_GRID,1 * H_GRID];
        _btnEndSearch ctrlCommit 0;
        _btnEndSearch ctrlsettext "\a3\3den\data\Displays\Display3den\search_end_ca.paa";
        _btnEndSearch ctrlAddEventHandler ["ButtonClick",{["endSearch", _this] call SELF;}];
        _edSearch = _map ctrlcreate ["RscEdit", IDC_RSCMAP_MRKSEARCH, _grpSearch];
        _edSearch ctrlsetposition [1.1 * W_GRID,0.1 * H_GRID,W_GRP - 2.3 * W_GRID,1 * H_GRID];
        _edSearch ctrlcommit 0;
        _edSearch ctrladdeventhandler ["KeyDown", {["keySearch", _this] spawn SELF;}];
        _edSearch ctrladdeventhandler ["SetFocus", {["focusSearch", _this] call SELF;}];
        _lbMarker = _map ctrlcreate ["RscListbox", IDC_RSCMAP_MRKLIST, _grpSearch];
        _lbMarker ctrlsetposition [0.1 * W_GRID,1.2 * H_GRID,W_GRP - 0.2 * W_GRID,H_GRP - 1.3 * H_GRID];
        _lbMarker ctrlcommit 0;
        addmissioneventhandler ["Map", {params ["_mapIsOpened", "_mapIsForced"];if (_mapIsOpened) then {["mapOpen", []] call SELF;};}];
        _lbMarker ctrladdeventhandler ["LBSelChanged", {["cityChanged", _this] call SELF;}];};
        case "mapOpen": {0 call _fncUpdate;["updateLoop"] spawn SELF;};
    	case "cityChanged": {params ["_lbMarker", "_ind"];_cityPos = call compile (_lbMarker lbData _ind);private _fixedScale = 0.05;mapanimadd [1, _fixedScale, _cityPos];mapanimcommit;};
        case "keySearch": {params ["_edSearch"];if (ctrltext _edSearch == _edSearch getvariable ["prevSearch", ""]) exitwith {};_edSearch setvariable ["prevSearch", ctrltext _edSearch];0 call _fncUpdate;};
        case "toggleVisibility": {params ["_cbShow", "_checked"];
        _checked = _checked == 1;
        if (_checked) then
        {
            {
